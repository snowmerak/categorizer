...
A FastAPI service that categorizes text with either [Qwen3-Reranker-0.6B](https://huggingface.co/Qwen/Qwen3-Reranker-0.6B) or [LFM2.5-Encoder-350M-Prompt-Router](https://huggingface.co/LiquidAI/LFM2.5-Encoder-350M-Prompt-Router). It also uses Qwen for response ranking in `/choices`. The environment is configured for Windows and NVIDIA GPUs with a CUDA 12.8 PyTorch build.

```powershell
uv sync
uv run python download_model.py      # Qwen reranker
uv run python download_embedding.py  # Optional Qwen prefilter
uv run python download_lfm.py        # LFM prompt router
uv run uvicorn main:app --host 127.0.0.1 --port 8000
```

Download only the models you plan to use. They are cached under `models/` and excluded from Git. Models load on their first request; an LFM-only request does not load Qwen. After downloading them, inference reads local files without contacting Hugging Face. Interactive API documentation is available at `http://127.0.0.1:8000/docs`.
...
Set `model` to `"qwen"` (the default) or `"lfm"` for each request. The selected model scores **every visited sibling group** using each candidate's full path and description. Qwen evaluates each candidate's yes/no suitability; LFM scores all siblings together in one encoder pass. LFM requests do not load or call either Qwen model. A child path's percentage is its parent's path percentage multiplied by that child's percentage at the current level, divided by 100. The search expands high-scoring nodes first to find the highest-ranked leaf paths. For example, a 60% parent with a 50% child yields a 30% path; a 40% parent with a 100% child yields a 40% path and ranks higher.

The response `model` contains the full model ID. Qwen candidate results retain `yes_logit`, `no_logit`, `yes_probability`, and `no_probability`; LFM candidate results omit those Qwen-specific fields. Both return `percentage`, which is relative to the siblings scored at that level, not a calibrated probability of correct classification. For LFM, include an `Unclassified` category if that outcome should compete with the other categories; a single candidate otherwise receives 100%.
...
For `model: "qwen"`, a level with **32 or more siblings** uses [Qwen3-Embedding-0.6B](https://huggingface.co/Qwen/Qwen3-Embedding-0.6B) to select the top 8 candidates before reranking them. Levels with 31 or fewer siblings go straight to the reranker, so they do not load the embedding model. Cache that model locally with `uv run python download_embedding.py`; it is stored in `models/qwen3-embedding-0.6b/` and excluded from Git. LFM always scores the full sibling group; prefilter options sent with `model: "lfm"` receive HTTP 422.
...
Cache the pinned LFM revision with `uv run python download_lfm.py`. Its custom model code is loaded with `trust_remote_code=True` from the local directory. The model currently runs in float32 because its routing method creates float32 pooling tensors. `LFM_DEVICE` can be `auto` (default), `cuda`, or `cpu`; auto selects CUDA when at least 2.5 GB is free, otherwise CPU. `LFM_MAX_LENGTH` limits the **combined** user text and sibling descriptions to 4,096 tokens by default. Oversized groups receive HTTP 422 rather than silent truncation. Since all sibling descriptions share one input, keep them concise for LFM.
